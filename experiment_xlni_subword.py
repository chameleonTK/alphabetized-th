...
if __name__ == "__main__":
  ...
  if len(sys.argv) > 1:
        ...
        wven = load_vectors(f"{wvname}/vectors-en.txt", normalised=False)

        thcol = sys.argv[2] if len(sys.argv) > 1 else "th"
        print("Loading TH wv", wvname, thcol)
        wvth = load_vectors(f"{wvname}/vectors-{thcol}.txt", normalised=False)
  ...
  for i in range(4, 11):
    ...
    if thcol=="th":
        thtokenizer = tokenizer.subwordTokenize
    else:
        thtokenizer = tokenizer.subwordTCCTokenize

    print("Tokenized sentence:", thtokenizer("แมวกินปลามหัศจรรย์"))
    args.epochs = 10
