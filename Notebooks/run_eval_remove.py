...
def transform_remove(sent, p, seed=42):
    random.seed(seed)
    tokens = list(sent)
...
class WordVector:
    def __init__(self, p_remove, fwd=True, bkw=True, model_dir_prefix="", model_dir_suffix=""):
        embs = []
        if fwd:
    ...
    def get_vector(self, word):
        newword = "".join(transform_remove(word, p=self.p_remove))
        
        cc = 0
        while len(newword)==0:
            newword = "".join(transform_remove(word, p=self.p_remove, seed=cc))
            cc += 1
...
for i in range(5):
    for p in range(1, 10):
        p_remove=p/10.0
        print(f"Remove {i+1}")
        wv = WordVector(p_remove=p_remove, model_dir_prefix=f"remove/{i+1}_", model_dir_suffix=f"_{p_remove}")
        wordsim = eval_word_sim(wv, expname=f"Remove {i+1}:{p_remove}:")